...
const app = express();
const port = 3000;
const AUDIT_TIMEOUT = 300000; // 5 minutes global timeout
...
const baseConfig = {
    ...
    settings: {
        throttlingMethod: 'simulate', // More realistic throttling
...
app.post('/audit', async (req, res) => {
    ...
    Promise.race([auditPromise(), timeoutPromise]).catch(err => {
        console.error('Timeout or audit error:', err.message);
        if (!res.headersSent) {
            res.status(504).json({error: 'Server process timed out. The task was too long to complete.'});
        }
    });
});
